defmodule Blitzy.Worker do
  ...
  defp handle_response({msecs, {:ok, %HTTPoison.Response{status_code: code}}})
    ...
    Logger.info "worker [#node#{node()}-#{inspect self()}] completed in #{msecs} msecs"
    {:ok, msecs}
  end

  defp handle_response({_msecs, {:error, reason}}) do
    Logger.info "worker [#node#{node()}-#{inspect self()}] error due to #{inspect reason}"
    {:error, reason}
  end

  defp handle_response({_msecs, _}) do
    Logger.info "worker [#node#{node()}-#{inspect self()}] errored out"
    {:error, :unknown}
  end
end
