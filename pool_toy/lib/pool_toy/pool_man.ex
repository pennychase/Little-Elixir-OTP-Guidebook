defmodule PoolToy.PoolMan do
  ...
  def init(size) do
    ...
    {:ok, %State{size: size, monitors: monitors}, {:continue, :start_workers}}
  end

  def handle_continue(:start_workers, %State{worker_sup: sup, worker_spec: spec, size: size} = state) do
    workers = 
      for _ <- 1..size do
        new_worker(sup, spec)
      end
